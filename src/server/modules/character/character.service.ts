...
export class CharacterService {
  ...
  public async create(
    ...
    await char.save();
    Logger.info(
      `[CharacterService] Caracter creat: ${char.firstName} ${char.lastName} pentru User ID ${userId}`
    );
    return char;
  }
  ...
  public async savePositionData(
    ...
    try {
      ...
      if (char) {
        ...
        await char.save();
        Logger.info(
          `[CharacterService] Poziția salvată pentru Char ID ${charId}`
        );
      }
    } catch (e: any) {
      Logger.error(
        `[CharacterService] Eroare la salvarea poziției pentru Char ID ${charId}: ${e.message}`
      );
    }
  }
}
