...
export class AuthSystem implements ISystem {
  ...
  private registerEvents(): void {
    mp.events.add(ServerEvents.AUTH_LOGIN, (player, username, pass) =>
      this.handleLogin(player, username, pass)
    );
  ...
  private async handleLogin(
    ...
    try {
      const result = await AuthService.getInstance().validateCredentials(
        username,
        pass
      );

      if (!result.success || !result.user) {
        return player.call(ServerEvents.AUTH_RESPONSE, [
      ...
      player.call(ServerEvents.AUTH_RESPONSE, [
        { success: true, characters: characterList, maxSlots },
      ]);
      Logger.info(`[Auth] ${username}(#${user.id}) s-a autentificat.`);
    } catch (e: any) {
      Logger.error("Eroare autentificare:", e.message);
      player.call(ServerEvents.AUTH_RESPONSE, [
        { success: false, error: "Eroare autentificare" },
      ]);
    }
  }
  ...
  private async handleRegister(
    ...
    try {
      const result = await AuthService.getInstance().createAccount({
        username,
        pass,
        email,
      });
      ...
      HUDUtils.update(player);
      Logger.info(`[Auth] Cont creat: ${username}(#${result.user.id})`);
    } catch (e: any) {
      Logger.error("Eroare inregistrare:", e.message);
      player.call(ServerEvents.AUTH_RESPONSE, [
        { success: false, error: "Eroare inregistrare." },
      ]);
    }
  }
}
